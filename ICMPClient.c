...
main (int argc, char **argv)
{
  ...
  src_ip = allocate_strmem (INET_ADDRSTRLEN);  //ip地址为16字节
  dst_ip = allocate_strmem (INET_ADDRSTRLEN);
  ip_flags = allocate_intmem (4);   //16个字节的flag
  ...
  // send packet.
  if (sendto (sd, packet, IP_HDRLEN+ ICMP_HDRLEN+ datalen, 0, (struct sockaddr *) &sin, sizeof (struct sockaddr)) < 0)  {
